...
- **연구 설계 원칙 2 (사용자 지시 2026-07-26)**: **강화학습 정책 최적화**는 핵심 정보만으로 먼저 구성하고, 효과가 실증된 뒤에만 부가 정보를 추가한다 — 정책에 넣는 정보(상태·보상 신호·후보)가 한꺼번에 많으면 어디서 문제가 생겼는지 디버깅할 수 없다 (한 번에 한 축씩, 각 추가는 자기 실증을 동반).
- **연구 진행 3대 게이트 (사용자 지시 2026-08-05)**: 다음 단계 전 `성능 개선·Dashboard↔코드↔증거 정합·시드 현실타당성`을 모두 판정한다. 미통과가 있으면 그 항목을 직접 고치는 단일축 작업만 허용하고, 전부 통과하면 관련 없는 새 가설을 만들지 말고 연구 목표의 확증·잠금평가로 진행한다 (절차: research-gates).
- Dashboard row 는 1줄 index 로 유지한다 — 상세 명세는 `.claude/docs/dashboard-task-specs/<ID>-*.md` 로 분리. **모든 row 는 완료 여부와 무관하게 spec 파일이 필수이고, row 가 갱신되면 같은 커밋에서 spec 도 갱신한다** (사용자 지시 2026-07-29 — 등록 시 spec 생성이 등록의 일부다. 1세대 Archive 완료분 소급은 YR-126).
