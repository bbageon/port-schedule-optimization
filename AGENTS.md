...
- **연구 설계 원칙 2 (사용자 지시 2026-07-26)**: 하네스 최적화는 **핵심 정보만으로 먼저 구성**하고, 효과가 실증된 뒤에만 부가 정보를 추가한다 — 정보(상태·신호·후보)를 한꺼번에 많이 넣으면 어디서 문제가 생겼는지 디버깅할 수 없다 (한 번에 한 축씩, 각 추가는 자기 실증을 동반).
