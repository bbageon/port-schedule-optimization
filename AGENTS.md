...
- **연구 설계 원칙 (사용자 지시 2026-07-26)**: 하네스·정책에 연구 피드백을 반영할 때 목적은 **최적화를 통한 연구적 개선**이다 — 하이브리드 결합·레이어 추가 같은 현실 공학적 우회는 되도록 쓰지 않는다 (단일 메커니즘을 더 잘 최적화하는 방향 우선; 레이어 추가는 결정주기 초단위화·다중블록 확장 같은 명시적 조건에서만 재검토).
