...
| **Evidence 박제** | Done 은 반드시 push 된 commit 해시/산출물 링크 동반. Cancelled 는 폐기 사유 동반 | "완료 주장"과 "검증 가능한 완료"를 분리. negative result 도 보존 |
| **순서 제시 원칙** | Agent 가 "다음 할 일·우선순위"를 말할 때 **board 에 등록된 row 만** 인용. 없는 작업은 먼저 backlog 등록 | Agent 의 즉흥 계획 난립 방지, 계획의 단일 출처 유지 |
| **훅으로 습관화** | 세션 시작·git commit 시점에 board 갱신과 origin push reminder 를 자동 주입 | 규칙을 문서에만 두면 잊힌다 — 하네스가 대신 기억 |
...
5. **작업 종료(commit + push)**: 검증 후 작업 범위만 commit 하고 `in-progress.md` → `done.md` 이동 + **commit 해시/링크를 Evidence 열에 박제**. Dashboard 변경도 commit 한 뒤 `origin` push 성공을 확인한다. 파생 후속 작업은 그 자리에서 `backlog.md` 에 등록.
...
- **작업 종료(commit + push)**: 검증 → 범위가 명확한 commit → in-progress → done + commit 해시/링크 박제 → Dashboard 변경 commit → origin push. 후속 작업은 backlog 에.
...
운영: 시작 시 [ready.md](ready.md) 에서 pull. 종료 시 [done.md](done.md) 로 이동 + commit 링크 박제 + origin push 성공 확인.
```
        ...
        "hooks": [
          {
            "type": "command",
            "shell": "bash",
            "command": "echo '{\"hookSpecificOutput\":{\"hookEventName\":\"SessionStart\",\"additionalContext\":\"[Project Board] 작업 시작 전 Dashboard 를 읽어라. 종료 시 검증→commit→done evidence→origin push 성공 확인까지 수행하라. upstream 이 없으면 git push -u origin HEAD. index=README.md.\"}}'",
        ...
        "hooks": [
          {
            "type": "command",
            "shell": "bash",
            "command": "cat | grep -q 'git commit' && echo '{\"hookSpecificOutput\":{\"hookEventName\":\"PostToolUse\",\"additionalContext\":\"[Project Board] commit 후 done evidence 를 갱신·commit 하고 origin push 성공까지 확인하라. upstream 이 없으면 git push -u origin HEAD. push 실패를 완료로 보고하지 마라.\"}}' || true",
...
- **PostToolUse(Bash + `git commit` 매칭)** — 커밋 직후 "done 이동 + evidence 박제 + origin push" reminder 주입 → 완료 처리와 원격 반영 누락 방지. 커밋이 아닌 Bash 호출은 `|| true` 로 조용히 통과.
