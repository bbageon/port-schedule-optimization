...
run () {   # $1 = 창(시간)
  ...
  PYTHONPATH=src "$PY" -m yard_rl.v4.train \
      ...
      --horizon-h "$h" --out "outputs/v4/horizon/$tag" \
      > "outputs/v4/horizon/$tag.log" 2>&1
...
for t in h3 h6; do
  echo "--- $t ---"
  grep -E "총 |검증|손실" "outputs/v4/horizon/$t.log" 2>/dev/null | tail -4
done
