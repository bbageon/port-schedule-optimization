...
9. **판정을 하네스에 보고한 직후에는 반드시 사용자에게 "쉬운 말 요약"을 함께 낸다**
   (사용자 지시 2026-08-06). 게이트 이름·상태값(`PASS`/`FAIL`/`INCONCLUSIVE`)만 던지지 말고
   **①무엇을 검사했는지 ②무엇이 바뀌었는지(어떤 게이트가 왜 열렸/막혔는지) ③그래서 지금
   할 수 있는 일과 못 하는 일**을 일상어로 적는다. 게이트 축 이름도 한 줄로 풀어 쓴다
   (신뢰성 = "기록이 서로 맞는가", 현실성 = "만든 상황이 실제 항만처럼 굴러가는가").
