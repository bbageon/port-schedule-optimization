...
|---|---|---|---|---|
| YR-080 | RL | 목적함수 문헌정합 재설계 — **본선측 항 필수 병기(KPI 위계)** + 정규화 우선 + 간섭 마스크화 + 항 축소 | 🟠 | **사용자 지시 (2026-07-20) — 반드시 이행**: 문헌 근거([목적함수 문헌근거](../docs/strategy-history/2026-07-20-목적함수-문헌근거.md)) 상 필수 4요건을 YR-071 목적 재정렬에 반영. ① **본선측(sts_wait·vessel_delay) 반드시 병기** — 항만 KPI 위계(선박 turnaround 1차·트럭 2차, Port Economics ch.10.2)상 트럭대기만 최적화하면 선석 생산성과 상충 ② 정규화를 가중치보다 먼저·기준정책 실측값 기준(Marler&Arora 2010: 지배=가중치×크기) ③ interference 는 보상서 빼 마스크로만(이중계상·"안전거리는 제약" 문헌) ④ 13항→4~5묶음 축소. YR-071 확정 전 게이트 |
...
| YR-075-c | RL | 재조작 목적지 **진짜 비용 상한** — 목적지-as-action(교사 rollout 실제 총비용 평가)로 헤드룸 재판정 | 🟠 | **신뢰성 재검토 파생 (2026-07-20)**: YR-075-a 오라클은 future_blocked 프록시(비용 미평가)라 "RL 무의미" 미증명. 배치 효과가 하류 혼잡·대기(83%)로 간접 전달되므로 rollout 기반 비용 최적 오라클로만 진짜 헤드룸 측정 가능. 통과 시 RL/풍부한 규칙 정당화, 부재 시 "규칙 충분" 확정. 아키텍처: 목적지-as-action 리팩터 필요(비자명) — [재검토](../docs/strategy-history/2026-07-20-YR-075a-신뢰성재검토-오라클상한성반박.md) |
