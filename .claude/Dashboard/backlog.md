...
| YR-062 | RL | 단일 DQN 성능 경로: BC(모방) 초기화 + RL 미세조정 — SF_SPT 초과 시도 | 🟠 | **YR-061 phase-3 파생**: 표현력 충분 확정·BC 56.25(종전 RL 대비 −20%, SF_SPT +3.13). 미세조정이 BC 를 되망가뜨리는지 자체가 신용 희석의 2차 검정. 단일야드 YR-030-b(greedy-prior) 교훈 참조 |
| YR-063 | RL | 단일 DQN 신용 개별화 — 팀 공유비용 균등귀속을 로컬/차분(counterfactual) 귀속으로 | 🟠 | **YR-061 원인 특정(TD 신용 희석) 파생**. QMIX 트랙(YR-013/059, 병행 세션)과 상보 — 이쪽은 단일 학습자 보상 귀속 축. YR-039 로컬 utility 잔존 코드 재사용 검토 |
