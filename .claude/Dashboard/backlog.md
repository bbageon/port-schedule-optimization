...
|---|---|---|---|---|
| YR-073-b | RL | 순위 증류 확증 재판정 — 선택 규칙 정정(사전 지정 DAgger 산출물)·guard 상대화·신규 seed | 🔴 | **YR-073 기각 파생 (2026-07-19, 사용자 결정 대기)**: 비선택 R1 이 사후 분석에서 전 핵심 통과(완주 20/20·REPO 정상·유의 개선 −1.5/−2.6분) — 남은 것은 "사전 지정 하 재현" 확증뿐. 학생·SF 만 비교(수 분 비용) + 소수 seed(3~4/20) 퇴화 완화(DAgger 2R·swa guard) 동반 — [분해](../docs/strategy-history/2026-07-19-YR-073-순위증류-prereg.md) |
