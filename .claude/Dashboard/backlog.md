...
|---|---|---|---|---|
| YR-072 | Exp | 천장 재진단 — 문헌보정 v2 프로파일 × 현실 부하(피크)에서 YR-070 재실행 | 🟠 | **D5·D1 결정 파생 (2026-07-19)**: "천장이 낮은 것은 한산한 시나리오 탓" 가설 검정 — 예측·조율 상금이 커지면 학습 트랙(YR-066) 재개 근거, 그대로 작으면 문제 성질 확정. YR-071(목적 축)과 상보 — [결정 기록](../docs/strategy-history/2026-07-19-YR-002-D1-D5-사용자결정.md) |
| YR-071 | RL | 목적함수 재정렬 — 운영품질(swa·대기·완료) 1차 목적화 후 천장 재측정 | 🟠 | **YR-070 천장진단 §5-A 파생**: 현 total_cost 는 "일 덜 하면 낮음"(cost↔health 오정렬 실측 — 탐색·QMIX 의 비용이득이 swa 하락 동반). 목적 정렬이 협조/QMIX(YR-013)·차분 트랙의 **선결** — 안 고치면 틀린 자로 ~9% 헤드룸 쫓음. 정렬 후 천장(YR-070) 재측정으로 "진짜 상금" 확정 |
| YR-066 | RL | 차분 신호 개량 — on-policy counterfactual base·창 확대·amortized-JR 회귀 | 🟠 | **우선순위 상향 (2026-07-19, YR-068 기각 근거)**: 본 시나리오에서 D_mean 급감(−5~−7→−1.6~−2.5) — 결정 밀도 상승이 창 내 반사실 차이를 희석. **규모에서 살아남는 차분 신호 설계가 협조 트랙의 선결 병목**. rollout 비용 주의 |
| YR-069 | RL | 차분 표적 QMIX 민감도 — λ_mix·창 그리드 (별도 사전등록) | ⚪ | YR-013c prereg 가 금지한 knob 탐색의 정식 경로 — **YR-068 기각으로 조건 변경: 차분 신호 회복(YR-066) 후에만 의미** |
