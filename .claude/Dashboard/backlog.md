...
| YR-064 | RL | 단일 DQN: BC 초기화 + 차분 신호 미세조정 — 효율(BC)×행동유인(차분) 결합 | 🟠 | **YR-063 파생**: TD(YR-062 파괴)와 달리 차분 1-step 은 순위-정렬 신호 — BC 를 보존·개선하는지 검정. 알려진 위험: CE 서수 Q→D 재척도화 (YR-062 §2 동형) |
| YR-065 | RL | 단일 DQN: 차분 신호 개량 — window 사다리(1200/2400s)·amortized-JR 회귀 | 🟡 | **YR-063 파생**: "순서를 모름"의 원인 후보(600s 근시·base_policy 불일치) 검정 축. rollout 비용 window 비례 증가 주의 |
