...
|---|---|---|---|---|
| YR-045 | Exp | YR-039 정정판 locked 재실험 — 신규 seed + ETA 두 경로 3-arm 분리 + §18 다중 게이트 | 🟠 | [spec](../docs/dashboard-task-specs/YR-045-corrected-locked-rerun.md) · [사전등록](../docs/strategy-history/2026-07-16-YR-045-corrected-locked-rerun-prereg.md) · 선결 YR-050 done(2026-07-17) — **남은 선결 = YR-053 실행환경(신규 머신 torch)** · 총비용 CI 단독 불채택 · 전략적 WAIT ablation arm 포함(YR-052) |
| YR-053 | Infra | 신규 머신(rjsdn) 개발환경 구성 — WSL torch venv + Windows venv(신버전 streamlit) | 🟠 | **YR-050 중 발견 (2026-07-17)** — 이 머신은 새 클론이라 [환경 문서](../docs/개발환경-windows-wsl.md)의 venv 2종이 없음. 현재 Anaconda py3.12 로 순수 파이썬 297 passed 이나 torch 8파일 실행 불가·UI 테스트 1건 구버전 streamlit API 비호환 실패. YR-045 학습 실행의 선결조건 |
