...
|---|---|---|---|---|
| YR-013 | RL | 중앙 공동배정·QMIX 다중 YC 협조 — **2차 판정(2026-07-19): 입력 표현 가설 기각 — 정규화 후 발산 서명은 소멸했으나 성능 +21~+29 악화. 재도전은 신용 축 종합 후** | 🟠 | [매핑 §6](../docs/YR-013-QMIX-매핑.md)·[YR-059 §4](../docs/YR-059-상태정규화-매핑.md): 사다리 OFF +19.2 열세 → 정규화 ON 은 133~139 로 더 악화 (INDEP 는 80.51 로 개선). 신용 희석(YR-061~065 3중 확증)이 mixer 에도 적용 — 차분/신용 축 결론 후 state_norm 결합으로 재판정. 구현물(mixer·사다리 하네스) 재사용 |
