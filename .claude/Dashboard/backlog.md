...
|---|---|---|---|---|
| YR-047 | Infra | 학습 3.6배 가속 — `_plan` 의 전체 스택 deepcopy 제거 (실측: 학습시간의 71.4%가 한 줄) | 🟡 | **실측 근거 (2026-07-16, WSL·프로파일러 왜곡 제거)**: 학습 10 에피소드 32.6s 중 `engine.py:287` `work=copy.deepcopy(self.stacks)` 가 **15,669회·23.3s·71.4%**. 후보 1건 계획할 때마다 야드 스택 전체를 복사 — 1회 1.2ms. 다음 후보는 `dry_run_commit`(3,205회) 인데 **0.1%** 로 무해(예약표만 복사). 제거 시 상한 **3.57배** → YR-045 학습 81분 → **약 23분**. 방향: 복사 대신 되돌리기(undo-log)나 덧씌우기(overlay) — 단 `_plan` 은 "스택 미변형" 계약이 정확성의 핵심이라 등가성 검증 필수(golden event hash 불변·기존 계약 테스트 통과가 게이트). **GPU 는 대안이 아니다** — torch 는 전체의 약 4%라 상한 1.04배 ([환경 §6](../docs/개발환경-windows-wsl.md)). YR-045 전 필수 아님(있으면 반복이 빨라질 뿐) |
