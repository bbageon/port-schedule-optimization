...
|---|---|---|---|---|
| YR-073 | RL | 중앙 공동가치망 — JR_NEW 교사 증류 (공동후보 순위 모방 + SF-SPT 대비 advantage 표적) | 🟠 | **개정 전략 주축 (2026-07-19, 사용자 피드백)**: 2크레인 공동조합을 중앙망 하나로 평가 — QMIX 신용 분할 문제 자체를 구조에서 제거하고, 느린 JR_NEW 탐색을 빠른 망으로 근사. G1=SF-SPT 유의 초과(학습정책 최초)·G2=JR_NEW 잔여 격차 보고·행동분포 guard(REPO 급증+SERVE 감소=퇴화 경고). 선결: YR-071 G0 통과 |
| YR-074 | RL | NEW 목적 반사실 미세조정 — 증류 정책의 잔여 격차 축소 (조건부) | ⚪ | **YR-062/064 교훈 게이트**: 미세조정이 교사 정책을 파괴할 수 있음 — ep0(증류 원본) 포함 val-best 선택으로 정직 보고, 표적은 NEW 목적 반사실(SF-SPT 기준선 대비). YR-073 성공 후에만 |
| YR-075 | RL | 결정권 확장 2단계 — 재조작 목적지·반입 장치위치 행동공간 추가 | 🟡 | **개정 전략 §9 (2026-07-19)**: 현 행동공간(SERVE·PRE_REHANDLE·REPOSITION)에서 G1 확정 후에만 개방 — 확장별 기여를 단계 측정. 아이디어 자체는 유효 유지 |
