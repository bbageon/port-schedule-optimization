...
|---|---|---|---|---|
| YR-013 | RL | 중앙 공동배정·QMIX 다중 YC 협조 — **1차 판정: 현 입력 표현에서 미채택, YR-059 선결 후 재판정** | 🟠 | [매핑 §6](../docs/YR-013-QMIX-매핑.md): 사다리(사용자 지시) 결과 안정화 후에도 4배 예산 @2000 에서 INDEP 대비 +19.2 유의 열세·WAIT 146 폭증. 예산 반응은 있어(1000→2000 −12) 구조 폐기는 아님. 구현물(mixer·사다리 하네스)은 재사용 |
| YR-059 | RL | 상태 feature scale-only 정규화 + 클리핑 → QMIX 재실행 | 🟠 | **QMIX 발산 1차 용의자 대응** ([적용전략](../docs/상태정규화-보상가중치-적용전략.md) §4·6-1). 필드별 동결 norm_ref(비용 scale 기계 재사용)·결측=0 보존·SCHEMA bump. YR-013 종속, 정규화 on/off ablation. **사다리 결과로 근거 강화 (매핑 §6)** |
| YR-060 | RL | QMIX 타깃 PopArt 보완 (return 크기 비정상성) | 🟡 | [적용전략](../docs/상태정규화-보상가중치-적용전략.md) §6-2 · YR-059 결과 조건부 (입력 정규화로 부족할 때만) |
| YR-061 | RL | 단일 DQN 보상 재설계 — 미서비스·미완료 과소처벌 제거 ("일 안 하면 손해") | 🟠 | [진단 판정](../docs/strategy-history/2026-07-18-단일DQN-11조건-진단-보상신용할당-판정.md): 11조건 학습기 진단 — 전 조건 SERVE 붕괴·REPOSITION 퇴화, 학습부/정규화/후보 knob 전부 무효. 원인 = 학습신호가 완료 제약 미반영(판정 게이트와 불일치). 1단계 = 진단 하네스 정식화(임시 스크립트 미보존) |
