...
| YR-077-2 | Sim | 돌발 강건성 2단계 — 장부 불일치(컨테이너 실제≠기록 위치) 모델 + 노쇼 통합 | 🟡 | **YR-077 1단계 통과 파생 (2026-07-20)**: 고장 축은 채택본 강건성 확인 완료 — 남은 미구현 축 = 장부 불일치(rollout 예측이 틀린 위치를 가정하는 진짜 예측 붕괴 시험)·노쇼(YR-019 연계). 신규 엔진 모델 필요 |
