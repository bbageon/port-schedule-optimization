...
|---|---|---|---|---|
| YR-151 | RL | Block PPO SELL — **0A 종료(계약 8종 20/20 통과)**, 0B 는 YR-150 선결 대기 | 🔴 | **0A 결과(2026-08-06)**: 기술 계약 8종 전 항 통과 — 후보 창 30분·**실현 미래 누출 0**(교란 검사)·PRE_GATE 원자 재배정(게이트 진입 시각 A 불변)·이송 1회 상한 엔진 fail-closed·stale KEEP·rollback 복원·장부 보존. 판정 `ae6005b`·spec 정합 `0e6727c`·[report](../../outputs/reports/yr151_pre_gate_0a/report.md). **하네스 보고 결과 신뢰성 게이트 FAIL → PASS**. **0A 에는 학습 GO/STOP 권한 없음** — 0B 는 YR-150 지속 유입 환경이 선 통과해야 착수. 발견 2건: ①기존 post-gate 경로의 이송 상한 엔진 미강제(후속) ②현 물리에서 블록별 게이트 거리차 0 → PRE_GATE 예측 route 비용 0(모형 한계 명시 대상) · **이전 선결 해소(2026-08-05)**: YR-149 5셀 데이터 자격 통과(`3844f8b`) — 하드 8/8·용량상태 확립·**가동률 A 91.9% vs B 42.1%(가동률 차 49.8%p·B 평균 비가동 57.9%)** 로 부하 불균형 환경 존재 확인. **주의**: 6시간 평균값이라 **SELL 결정 순간의 여유를 뜻하지 않는다**(29차) — 순간 여유는 0A/0B 에서 직접 측정. **★30차 — 0A/0B 분리**: **0A(완료)** = 기술 계약 검사만 — 후보 생성·30분 결정창·미래정보 누출 0·**PRE_GATE 원자 재배정**(현 엔진은 gate-in 전 이송 거절). **0A 에는 학습 GO/STOP 권한이 없다**(후보·상금이 적어도 유한 물량 구조 특성일 수 있음). → **YR-150 지속 유입 환경 구축** → **0B**(지속 환경에서 실제 SELL 상금·공개정보 이득 부호 구분·결정 **순간** 수신 여유 재검사) — **학습 GO/STOP 은 0B 에서만**, 미달 시 `NO_LEARNABLE_SIGNAL`. 통과 시 Q30 동결 → shadow → on-policy 학습 → K/Q30/S 3군 · [spec](../docs/dashboard-task-specs/YR-151-block-ppo-sell-head.md) |
