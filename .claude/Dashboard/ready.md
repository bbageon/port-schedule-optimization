...
|---|---|---|---|---|
| YR-151 | RL | **Block PPO TransferHead — 30분 ETA 반입 SELL/KEEP 학습** | 🔴 | **선결 해소(2026-08-05)**: YR-149 5셀 데이터 자격 통과(`3844f8b`) — 하드 8/8·용량상태 확립·**A 92% vs B 42% 유휴 공존 정량화**로 SELL 전제 성립. **0단계 = 측정 먼저(29차)**: ①30분 전 실제 SELL 후보가 얼마나 생기는지 ②그 순간 수신 블록에 실제 여유가 있는지 ③PRE_GATE 원자 재배정 구현 ④KEEP 대비 이송의 실제 개선 여지 ⑤공개정보로 이득 부호 구분 가능성 — **신호 없으면 `NO_LEARNABLE_SIGNAL` 로 학습 중단**. 통과 시에만 Q30 동결 → shadow → on-policy 학습 → K/Q30/S 3군. **주의**: YR-149 5셀은 유한 물량 회복 구조이므로 지속 성능 확증은 YR-150(지속 유입) 구조를 따른다 · [spec](../docs/dashboard-task-specs/YR-151-block-ppo-sell-head.md) |
