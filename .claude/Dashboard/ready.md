...
|---|---|---|---|---|
| YR-171 | Exp | **시간 판매 → 당일 30분 슬롯 최적화 계약 (YR-161 대체)** | 🔴 | **보정 대상**: `scenario_validity` — 정보 계약 변경이라 **재자격 필수**. **왜**: 축 분리 실측에서 시간 판매만 +438.3(+18.68%·1건당 +14.28분) 손해. 원인은 기사 외부 대기가 내부 대기와 같은 단가라 이연 순이득 = 절약분 − 15분인데 **고정 +15분은 혼잡 첨두를 고를 수 없다**. **무엇**: 30분을 '보는 범위'가 아니라 '예약 슬롯 크기'로 쓰고, 같은 작업일 48개 슬롯 중 최저비용 슬롯을 resolver 가 공동 배정(슬롯 용량 차감·임박 잠금·날짜 넘김 금지). 근거: 부산항 체인포털 VBS 구조(00~24시·30분/1시간 슬롯·변경 가능시간과 마감 분리). **단서**: 예약 최적화≠재예약(기사 동의 필요) — 현 시뮬은 전원 수락 가정이므로 **최대 가능 효과**로만 해석 · [spec](../docs/dashboard-task-specs/YR-171-time-sale-slot-contract.md) |
