...
|---|---|---|---|---|
| YR-151 | RL | **Block PPO TransferHead — 30분 ETA 반입 SELL/KEEP 학습** | 🔴 | **선결 해소(2026-08-05)**: YR-149 5셀 데이터 자격 통과(`3844f8b`) — 하드 8/8·용량상태 확립·**A 92% vs B 42% 유휴 공존 정량화**로 SELL 전제 성립. **0단계 = gate-in 전 원자 재배정(PRE_GATE) 구현**(현 엔진은 gate-in 전 이송 거절) → Q30 계산 기준선 동결 → shadow 배선 검증 → on-policy 학습 → K/Q30/S 3군. 계산견적 우월은 선결 아님(잔여는 YR-152) · [spec](../docs/dashboard-task-specs/YR-151-block-ppo-sell-head.md) |
