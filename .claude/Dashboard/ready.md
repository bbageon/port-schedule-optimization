...
|---|---|---|---|---|
| YR-168 | Exp | **5차 계약 무대에서 공간 판매 재판정(YR-166 흡수)** | 🔴 | **착수 가능**(YR-167 재자격 12/12 통과 273ffc5 — 무대 신뢰성 확보). **무엇**: 벌금 구간에 실제로 도달하는 24시간 이중 피크 무대에서 블록 간 공간 판매의 반사실 짝비교를 다시 판정한다. **왜 지금**: 0B 판정이 STOP 이었던 직접 원인은 무대가 벌금 구간(43분)에 못 갔다는 것(실측 체류 20분)이고, YR-167 자격에서 피크 도달률 24.0% 가 확인돼 그 전제가 해소됐다. 투입 고정(YR-166)은 개방 루프 스케줄러로 계약에 내장돼 별도 축이 불필요하다. **선행**: 판정 임계·표본수 사전등록 동결 후 실행 · [spec](../docs/dashboard-task-specs/YR-168-sale-rejudgment-diurnal.md) |
