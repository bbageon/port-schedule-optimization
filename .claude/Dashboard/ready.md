...
|---|---|---|---|---|
| YR-150 | Exp | **21블록 터미널 전체 지속 유입 정상상태 자격시험** | 🔴 | **현재 허용된 `scenario_validity` 보정**. N=3에서 다수 수신처·목적지별 route·rollback·matching을 먼저 검사한 뒤, N=21에서 하나의 terminal master stream을 `p`로 배분한다. `50·75·100·125·150`은 전체 4시간 유입량이며 혼잡등급이 아니다. 지금 범위는 환경 자격까지이고 정책 성능 비교는 현실성 PASS 후 별도 승인한다 · [spec](../docs/dashboard-task-specs/YR-150-continuous-inflow-steady-state.md) · [결정](../docs/strategy-history/2026-08-06-YR-150-151-터미널전체-21블록-사용자결정.md) |
