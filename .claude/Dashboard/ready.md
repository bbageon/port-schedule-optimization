...
|---|---|---|---|---|
| YR-105-b | RL | **상대 혼잡격차 임계 정책 최적화 (`0.05/0.10/0.20` 단일축)** | 🔴 | **YR-116 선결**. YR-113이 현행 0.10 규칙의 트럭 이득을 확증했으므로, `C_zero(src)−C_zero(dst)`의 임계만 바꿔 **언제 이송할지** 최적화한다. 신규 파일럿으로 표본을 다시 산출하고 선택→독립 확증한다. 절대혼잡 `C_HIGH`나 지속조건은 섞지 않는다. |
