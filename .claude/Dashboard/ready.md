...
|---|---|---|---|---|
| YR-168 | Exp | **5차 계약 무대에서 공간 판매 재판정(YR-166 흡수)** | 🔴 | **착수 가능**(YR-167 재자격 12/12 통과 273ffc5). **보정 대상**: `performance` 하나. **무엇**: 벌금 구간에 도달하고 판매 기회가 실측으로 존재하는 24시간 이중 피크 무대에서 블록 간 공간 판매를 반사실 짝비교로 다시 판정한다. **왜 지금**: 0B STOP 의 기록된 사유는 **견적식 구분력 부족**(축2 순위상관 −0.204)이고, "벌금 구간 미도달이 STOP 의 원인"이라는 인과는 **검증된 적이 없다**(40차 감사 — YR-164·165 는 체류시간을 측정조차 안 했다). 정확히는 **저부하 + 투입기 오염으로 판정의 정답 라벨이 오염**(T_other +16.52 > 총이득 +14.95)돼 그 STOP 을 새 환경에 이월할 수 없다. 해소된 것은 무대 전제와 투입기 오염(개방 루프 계약 내장 — YR-166 흡수), **축2 견적 구분력은 미해소로 승계**. **관찰 모드 실측(3시드)**: 판매 후보 265시각 중 257(97%) 존재·후보 보유 블록 평균 10.5, 기회 격차 평균 5.45~6.05(최대 21) — **피크[11,16)시 격차≥5 가 100%·야간[2,8)시 0%**. **선행(미완)**: ①시간대 정의 곡선 유도 동결(야간 [2,8)시 정정 완료·피크 재유도 필요) ②완주 하드 가드(3,600 완주·본선 완주·누락/취소 0 을 판정 조건으로) ③**눈가림 파일럿으로 5차 σ 재측정 후 표본 수 동결** — 4차 σ 기준 0.5h 검출에 63~98쌍, 견적 중앙값 0.139h 검출에 814~1,497쌍(과거 40쌍은 검정력 부족) · δ 는 사용자 결정 · [관찰 산출물](../../outputs/reports/yr168_observe/observe.json) · [spec](../docs/dashboard-task-specs/YR-168-sale-rejudgment-diurnal.md) |
