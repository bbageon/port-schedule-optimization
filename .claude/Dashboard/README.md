...
- **YR-048 ETA 경로 복구 (2026-07-16)**: 통합 시나리오 외부트럭에 ETA ±300초를 전용 난수 흐름으로 주입해 PRE_REHANDLE 후보를 0%→14~15%로 활성화. 동시에 ETA가 REPOSITION 후보도 3.3배 늘린다는 두 번째 경로를 확인해 YR-045를 `NO_ETA/ETA_NO_PRE/FULL` 3-arm으로 사전등록했다. SERVE 없는 한산기에 결정이 안 열리는 구조는 [YR-050](../docs/dashboard-task-specs/YR-050-eta-decision-epoch.md)으로 분리했다.
...
- **남은 실행 순서 (정정 트랙)**: **다음 = YR-050 ETA 결정 시점 보완** → YR-045 locked 재실험(신규 seed·ETA 두 경로 3-arm 분리·전략적 WAIT ablation(YR-052)·§18 다중 게이트) → YR-042(프로파일 일반화)·YR-013(QMIX, **착수 조건 미충족 회귀**) → YR-014. YR-050이 끝나기 전에는 조기 선제작업 효과를 완전히 판정할 수 없다. 실측·validation 은 YR-002/009. **열린 문제·대응 종합**: [열린문제 전략](../docs/strategy-history/2026-07-16-열린문제-대응전략.md).
