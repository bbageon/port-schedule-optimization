...
- **다중 블록 범위 확정 (2026-07-26, 사용자)**: TOS는 연구 밖의 최초 작업·블록 배정자이며 수정·대체하지 않는다. 터미널 확장의 기본은 공유 BlockPolicy를 블록별 독립 배포하는 구조다. 추가 결정권은 재배정 허용 반입에 한해 BlockQ가 판매가치·수용비용을 내고 결정론적 `TransferResolver`가 `KEEP/A→B`를 원자적으로 확정하는 것뿐이다. 수신자 없음·실패는 원블록 `KEEP`, owner 공백은 금지. **QMIX는 resolver가 아니며 초기 구조에서 제외**한다. [사용자 확정 이력](../docs/strategy-history/2026-07-26-YR-081-배정후-반입재배정-중앙-resolver-사용자확정.md) · [YR-099 spec](../docs/dashboard-task-specs/YR-099-post-tos-inbound-transfer-resolver.md)
...
- **현재 정책 후보 구조 (2026-07-26 갱신)**: 성능 트랙 = **공개정보 예측 rollout 단일 계층**(YR-096, 하이브리드 기각 — 연구 설계 원칙 1·2는 AGENTS.md). **RL-본선 트랙은 5축 소진 경계발견으로 종결**([종결 문서](../docs/strategy-history/2026-07-26-RL본선트랙-종결-경계발견-5축소진.md)) — RL의 보존 성과는 트럭 축(전 시드 유의·zero-shot 일반화·점증비용 제약 내재화), 본선 여지는 전부 적하·얇음(YR-098: VF −2.84)·rollout이 3~4배. **H1 목적지 규칙은 새 계약·물리에서 greedy 와 무차별(우월 주장 철회, YR-075-c)** — 목적지 상금(−4.54)은 rollout 평가 축에 있음. 시간계약 v2(YR-089: 학습 B−C·최종 A−O)·물리 정정(YR-091/092: 비통과·pile 규격, 골든 재동결) 완료 — rollout 우위는 정정 후 재판정으로 유지 확인(단 본선 이득 축소·현실형@3600 재검이 YR-096 관문). QMIX는 "다중 블록이라서" 자동 재개하지 않는다(YR-069 조건 축소 유지) |
- **장기 운영화 북극성 범위 축소 (2026-07-26, 사용자 정정 — 현재 범위 아님)**: 외부 TOS → 블록 정책들 → 좁은 반입 `TransferResolver`가 운영 구조다. 독자적인 터미널 통합계획기와 TOS 배정 재구축은 제외한다. LLM(YR-084)은 훗날 읽기전용 예외 해석·결정 설명·운영자 에스컬레이션만 맡고 TOS·resolver·장비를 통제하지 않는다. Level 3 운영로그·Level 4 Shadow 범위 승인 전에는 구현하지 않는다. [장기 명세](../docs/dashboard-task-specs/YR-084-llm-terminal-orchestration.md)
