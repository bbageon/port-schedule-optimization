...
- **★아키텍처 세대 구분 (2026-07-30, 사용자 정정 3)**: 정책 경계는 **YR-099**다. 외부 TOS가 각 블록의 Block Q에 작업을 직접 최초 배정하고, 블록별 Q는 자기 작업목록과 크레인 실행을 관리한다. 재배정 가능한 미적재 작업만 **Block Q↔TransferResolver** 양방향 견적을 거친다. Block Q는 작업종류·스케줄 정보를 볼 수 있지만 “본선이면 무조건 우선” 같은 유형 고정규칙은 쓰지 않는다. SELL은 Block Q 소유 행동이되 크레인 물리 후보와 분리된 작업목록 관리 head에서 결정한다. 1세대(~YR-098)의 성공은 구 지표·구 목적 위라 이월 금지이며, 2세대 BlockQ-v2와 견적 resolver는 아직 최종 검증 전이다. [세대 정정본](../docs/strategy-history/2026-07-29-아키텍처-세대구분-정정-YR-099경계.md) · [최신 로드맵](../docs/strategy-history/2026-07-30-BlockQ-v2-로드맵-10단사다리.md)
...
- **다중 블록 범위 확정 (2026-07-30, 사용자 정정 3)**: TOS는 연구 밖의 최초 배정자이고 공유 BlockPolicy를 여러 블록에 배포한다. 원소유 Block Q가 판매효과를 계산해 발의하고, 수신 Block Q가 수용부담을 응답하면 결정론적 `TransferResolver`가 `KEEP/A→B`를 원자 확정한다. 갱신은 **유의한 사건 즉시 + 사건이 없어도 최대 미갱신시간 `T_refresh` 타이머**로 연다. 타이머는 견적 재검토일 뿐 이송 명령이 아니다. 첫 구현은 현재 물리가 지원하는 미적재 `GATE_IN` 반입·epoch당 1건이며, 양하는 별도 안벽 인계 트리거를 정의한 뒤 확장한다. 수신자 없음·실패는 원블록 KEEP, owner 공백은 금지한다. QMIX는 resolver가 아니다. [YR-133 spec](../docs/dashboard-task-specs/YR-133-blockq-sell-quote.md)
...
- **개정 전략 사다리 (2026-07-23)**: YR-087 rollout 후보 탐색 뒤 **YR-089 시간계약 구현·SF-SPT/공동 rollout 재판정 → YR-075-c → 정책 bundle → YR-041** 순이다. ETA 오차·극단부하 YR-019·021은 실증 단계로 보류한다. YR-082-A 공개 registry는 병행하고, **정책 bundle + YR-082 Level 2 프로파일 2개 + YR-083 런타임 지원 → YR-042 → YR-014**로 합류한다. 크레인 수·다중 블록은 이후 YR-081에서 확장한다. [일반화 전략](../docs/strategy-history/2026-07-20-일반화-3단계-전략-사용자승인.md) · [열린문제 전략](../docs/strategy-history/2026-07-16-열린문제-대응전략.md).
