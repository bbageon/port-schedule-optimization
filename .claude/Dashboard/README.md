...
- **재배정 트랙 현재 순서 (2026-07-28)**: YR-099-b 원자 이송 브리지와 YR-112/116 교착·리뷰창 계약은 완료됐다. YR-113의 현행 0.10 이송은 이송 0보다 유망했으나, YR-117이 truck 비용의 블록도착 시계가 이송 주행을 빠뜨릴 수 있음을 확인했다. 현재 **YR-105-b**는 `0.05/0.10/0.20`을 **총비용+A→O 공동 주지표**로 최적화한다. 이후 **YR-115**가 동결 승자와 이송 0을 독립 확증하고, 필요할 때만 YR-102 quote로 간다. 절대혼잡·지속조건·본선필터를 동시에 섞지 않는다.
...
- **현재 정책 후보 상태 (2026-07-28)**: 공식 배포 후보는 아직 없다. 현행 0.10 이송은 YR-113에서 truck `+3.35`, YR-117 사후 재해석에서 A→O 약 `+0.84분`·확증대역 total `+8.36`의 유망 신호를 보였지만, 새 공동 주지표의 사전등록 확증은 아니다. YR-105-b가 임계 하나를 total+A→O로 최적화하고, YR-115가 최종 임계 대 이송 0의 순효과를 검증한다. QMIX·추가 관제층은 넣지 않는다.
