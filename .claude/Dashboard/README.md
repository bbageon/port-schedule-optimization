...
- **현재 정책 후보 구조**: YR-087에서 feed-forward 증류는 미래 rollout을 재현하지 못해 공식 채택 경로에서 빠졌고, 현재 성능 선두는 H1 목적지 규칙을 쓴 **2크레인 중앙 3600초 공동 rollout**이다. 실행은 resolver가 안전·물리 제약을 보장한다. 다만 이 후보의 트럭 성과도 구 `S-B` 지표이므로 YR-089 전에는 최종 정책으로 동결하지 않는다. QMIX는 다중 블록·다중 크레인에서 공동조합이 커질 때만 재검토한다.
- **장기 운영화 북극성 (2026-07-22, 사용자 결정 — 현재 범위 아님)**: `블록 정책 → 야드 계층관제(YR-081) → 결정론적 터미널 통합계획기 → LLM 감독 관제(YR-084)` 순으로 확장한다. LLM은 실시간 장비제어·안전판단을 하지 않고, 비정형 예외 해석·승인된 재계획 도구 호출·운영자 설명을 맡는다. Level 3 운영로그·Level 4 Shadow 승인 전에는 구현하지 않는다. [장기 명세](../docs/dashboard-task-specs/YR-084-llm-terminal-orchestration.md)
