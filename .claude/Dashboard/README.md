...
- **남은 실행 순서 (정정 트랙)**: **다음 = YR-050 ETA 결정 시점 보완** → YR-045 locked 재실험(신규 seed·ETA 두 경로 분리·§18 다중 게이트) → YR-042(프로파일 일반화)·YR-013(QMIX, **착수 조건 미충족 회귀**) → YR-014. YR-050이 끝나기 전에는 조기 선제작업 효과를 완전히 판정할 수 없다. 실측·validation 은 YR-002/009.
