...
- **★아키텍처 세대 구분 (2026-07-29, 사용자 정정)**: 정책 아키텍처의 경계는 **YR-099** (2026-07-26 사용자 확정 — 중앙 TransferResolver 가 재배정 가능한 반입·본선 양하를 블록에 배치하고, **단일 블록 Q 는 본선/트럭 유형을 모른 채** 계산된 비용으로 실행순서만 결정; 동반 YR-100 본선 비용 계산·YR-081 범위). 1세대(~YR-098, 라벨 직접 노출 단일블록 Q·본선 학습 접근)의 성공(YR-063·073-b·074)은 구 지표(S→B)·구 목적 위라 **이월 금지**. **2세대 위 학습 전적 = CALC 1건 guard 탈락·resolver 학습 미착수** — YR-119/121/122 3연속 기각은 1세대식 정책을 2세대 계측으로 진단한 것(2세대 아키텍처의 기각이 아님). 측정계약 정비(YR-080 목적·YR-089 시간·YR-104~117 판정)는 **별도 축**. [정정본](../docs/strategy-history/2026-07-29-아키텍처-세대구분-정정-YR-099경계.md)
