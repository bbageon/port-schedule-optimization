...
- **6단계 파이프라인 완료·후속 진단 반영 (2026-07-27)**: [1] YR-100 본선 비용 계산식 4계약은 검증 통과. [2] 현재 반응형/CALC 본선 타이밍은 3개 학습시드 중 1개만 유의해 재현성이 없었지만, 긴 lookahead·목적지까지 포함한 단일블록 계획 전체를 닫은 것은 아니다. [3] JR1800은 total 무의·berth 악화였고 보고된 P95 −10.5는 구 `S→B` 진단값이다. [4] **YR-041-a는 P95가 아니라 berth 비열등 실패로 기각**돼 배포 후보가 없다. [5] YR-099 오라클 −22.41은 제한된 완벽정보 잠재 상한이고 공개정보 quote는 +3.08로 실패했다. YR-101의 K=5 평균도 +3.54·실현개선 3/8로 기각돼, 단순 표본 확대가 아니라 저분산 비용분해 quote(YR-102)를 검토한다. [6] YR-095는 실제 자료 전까지 DATA-BLOCKED다. done.md YR-101·100·099-G1·041-a 참조
...
- **현재 정책 후보 상태 (2026-07-27)**: In Progress와 Ready는 비어 있고 **공식 배포 후보도 아직 없다**. YR-101 ensemble quote는 기각됐고, 다음 등록 후보는 YR-102 저분산 비용분해 quote와 YR-096의 남은 현실형 @3600 재검이다. YR-100은 계산계약 완료이지 정책 승리가 아니다. 보존 증거는 트럭 RL의 과거 `S→B` 개선, YR-089의 rollout `B→C` 상금, 적하의 얇은 타이밍 여지(YR-098), 목적지 상금(YR-075-c), 제한된 오라클 재배정 상한(YR-099)이다. 새 후보가 생겨야 YR-041을 재집행하며, QMIX는 다중 블록이라는 이유만으로 재개하지 않는다.
