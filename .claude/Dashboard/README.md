...
- **다중 블록 범위 확정 (2026-07-26, 사용자 정정)**: TOS는 연구 밖의 최초 작업·블록 배정자이며 수정·대체하지 않는다. 터미널 확장의 기본은 공유 BlockPolicy를 블록별 독립 배포하는 구조다. 현재 추가 결정권은 **아직 적재되지 않은 재배정 허용 반입·본선 양하(STORE)**에 한해 BlockQ가 제거효과·수용부담을 내고 결정론적 `TransferResolver`가 `KEEP/A→B`를 원자적으로 확정하는 것이다. 수신자 없음·실패는 원블록 `KEEP`, owner 공백은 금지. 지정 반출·본선 적하·후보선택·이미 적재된 실물 이동은 제외하고, 중량·위험물·냉동·선사·그룹 규칙(YR-095)은 **최종 현실화 단계**로 둔다. **QMIX는 resolver가 아니며 초기 구조에서 제외**한다. [최신 사용자 결정](../docs/strategy-history/2026-07-26-YR-081-반입-양하-우선-현실적재규칙-후순위.md) · [YR-099 spec](../docs/dashboard-task-specs/YR-099-post-tos-inbound-transfer-resolver.md)
- **YR-100 본선 계산계약 완료 (2026-07-27)**: `planned_completion_s` 기준, `YC→YT→안벽버퍼→STS` 공급경로, 같은 공개정보의 KEEP/TRANSFER 반사실, 미래진실 차단을 구현·검증했다. 다만 단일블록 실행타이밍 학습은 시드 재현성이 없었고, 이 계산식의 현재 주용도는 YR-099/102 transfer quote의 저분산 비용 원료다. [YR-100 spec](../docs/dashboard-task-specs/YR-100-vessel-cost-formula.md)
...
- **장기 운영화 북극성 범위 축소 (2026-07-26, 사용자 정정 — 현재 범위 아님)**: 외부 TOS → 블록 정책들 → 좁은 반입·양하 STORE `TransferResolver`가 운영 구조다. 독자적인 터미널 통합계획기와 TOS 배정 재구축은 제외한다. LLM(YR-084)은 훗날 읽기전용 예외 해석·결정 설명·운영자 에스컬레이션만 맡고 TOS·resolver·장비를 통제하지 않는다. Level 3 운영로그·Level 4 Shadow 범위 승인 전에는 구현하지 않는다. [장기 명세](../docs/dashboard-task-specs/YR-084-llm-terminal-orchestration.md)
