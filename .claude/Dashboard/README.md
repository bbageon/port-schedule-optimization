...
- **YR-100 착수 전 정정 게이트 (2026-07-26)**: 본선 계산식은 아직 구현 가능한 계약이 아니다. ① ETD 초안을 현 `planned_completion_s` 기반 본선비용·berth KPI와 맞추고 softplus surrogate·미완료비용을 공식 평가비용에 연결 ② `YC→YT→안벽버퍼→STS` 공급경로 포함 ③ 같은 공개정보·지평의 `KEEP vs TRANSFER` 예상 본선 완료비용 비교 ④ 현재 LOAD에서 숨겨지는 계획완료시각을 정책에 공개할 근거를 정해야 한다. 네 항목을 단일 블록에서 검증하기 전 ExecutionQ·TransferResolver에 연결하지 않는다. [YR-100 spec](../docs/dashboard-task-specs/YR-100-vessel-cost-formula.md)
...
- **트럭 시간계약 승인 (2026-07-23, YR-089)**: VBS 예약시각은 정책이 미리 보는 정보, 실제 gate-in은 비용 시작, 예상 블록도착은 예약·준수오차·게이트/내부이동으로 만든 파생 ETA다. 블록 정책은 `실제 블록도착→작업완료(B-C)`로 학습하고, 최종 채택은 `실제 gate-in→gate-out(A-O)` 평균·P95 턴타임으로 판단한다. 현재 `provided_eta=실제 블록도착±오차`, `truck_wait=S-B`, 고정 출문시간 근사는 의미가 달라 [상세 명세](../docs/dashboard-task-specs/YR-089-truck-time-contract.md)에서 분리 구현·재평가한다.
...
- **현재 정책 후보 상태 (2026-07-26 정정)**: Ready와 In Progress는 비어 있다. YR-096 공개정보 rollout은 “만든다면 단일계층”이라는 구조만 정했을 뿐 착수 확정이 아니어서 Backlog로 하향했고, YR-100 계산비용+잔여Q도 위 4개 계약을 고칠 Backlog다. 따라서 **공식 배포 후보는 아직 없다**. 보존 증거는 트럭 RL의 신뢰학습, 적하의 얇은 타이밍 여지(YR-098: VF −2.84), rollout의 더 큰 상금, 목적지 상금(YR-075-c: −4.54)이다. YR-096·100의 순서/비교관계 재확정 뒤 YR-041로 잠금 평가하며, QMIX는 다중 블록이라는 이유만으로 재개하지 않는다.
