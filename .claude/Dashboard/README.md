...
- **남은 실행 순서 (정정 트랙, 사용자 확정)**: **다음 = YR-045 locked 재실험** (§18 다중 게이트·신규 seed — YR-043 정정 비용 + YR-044 baseline 위에서, 선결조건이던 torch 환경 확보됨) → 그 후 YR-042(프로파일 일반화)·YR-013(QMIX, **착수 조건 미충족 회귀**) → YR-014. 실측·validation 은 YR-002/009.
