...
- **남은 실행 순서 (정정 트랙, 사용자 확정)**: YR-043 목적함수·행동공간 정정 (imbalance Load 재정의·per-term fit scale·WAIT 행동·물리 mask·가우시안 평균조건·λ_vessel=1.0 중립) → YR-044 Joint Immediate-Cost Greedy·beam baseline → YR-045 locked 재실험 (§18 다중 게이트·신규 seed) → 그 후 YR-042(프로파일 일반화)·YR-013(QMIX, **착수 조건 미충족 회귀**) → YR-014. 실측·validation 은 YR-002/009.
