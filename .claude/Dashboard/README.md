...
- **YR-002 재기준화 — D5·D1 사용자 확정 (2026-07-19)**: 운영사 협약 트랙 **폐기** (개인 연구자 실행 불가 — 사용자 확인) → **문헌 보정 프로파일 v2** ("신항 표준 ARMG" — HJNC·DGT 공개 스펙 종합, YR-022~042 yaml 재사용) + 부하 현실화(문헌 4~10대/h/크레인·피크) + **공개 turn-time 대조**(YR-009 재정의 1차) + 천장 재진단(YR-072). 연구 정체성 = 문헌 보정 시뮬레이션 방법론 기여. [결정 기록](../docs/strategy-history/2026-07-19-YR-002-D1-D5-사용자결정.md).
- **남은 실행 순서 (2026-07-19 갱신)**: ~~YR-068 확전~~ (**기각** — 차분 신호 규모 급감, done 참조) → 현재 **YR-002 재기준화** 집행 → 결과 따라 YR-071(목적 재정렬)·YR-072(천장 재진단 v2)·YR-066(차분 개량) 우선순위 재조정 → YR-042 재개 → YR-014. **열린 문제·대응 종합**: [열린문제 전략](../docs/strategy-history/2026-07-16-열린문제-대응전략.md).
