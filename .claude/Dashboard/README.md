...
- **개정 전략 사다리 (2026-07-19, 사용자 승인 — 조건 7건)**: 병렬 세션 탐색(YR-076)이 OLD 목적의 간섭 항 지배와 NEW 재정렬 시 실대기 급감을 발견 — "헤드룸 부재 = 문제 성질"(YR-070/072)은 **OLD 목적 기준의 판정**으로 정정. [승인 기록](../docs/strategy-history/2026-07-19-목적재정렬-개정전략-사용자승인.md). 사다리: ~~YR-071~~(**G0 통과**) → ~~YR-073~~(1차 기각) → ~~**YR-073-b**~~(**확증 통과 — 트랙 최초 학습정책의 규칙 초과, 아래**) → ~~YR-074~~(**미세조정 — 간섭 −31~35% 유의, FT 채택 전환**) → ~~YR-077~~(**돌발 강건성 1단계 통과 — 채택 유효성 3축 종료**). 다음 후보 = YR-075-a(재조작 목적지 확장)·YR-077-2(장부 불일치·노쇼)·전문가 반복 루프(등록 필요). 병렬 축: ~~YR-078~~(완료 — 교사 창 30분). YR-066 재정의·YR-069 조건 변경. 공통 후속 = 장치율 현실화 → YR-042 재개 → YR-014. **열린 문제·대응 종합**: [열린문제 전략](../docs/strategy-history/2026-07-16-열린문제-대응전략.md).
...
- **YR-074 미세조정 + YR-077 강건성 (2026-07-20) — 채택 유효성 3축 종료**: 반사실(보상) 미세조정이 대기 동급 유지 하에 간섭 −31~35% 유의 제거(계층 지배) → **FT 채택 전환** (사용자 승인). 고장 주입(크레인 15분 정지 1·2회)에서 이득 유지·오히려 강도↑ 격차↑(mid −1.84/−2.07·high −3.80/−4.09분) — rollout 증류의 예측 붕괴 우려 기각. **최종 채택 사슬 = 계층목적 계획기(JR_NEW 30분) 순위 증류 + DAgger + 반사실 RL 미세조정 → 성능·품질·강건성 3축 통과** (문헌 보정 조건). [YR-074](../docs/strategy-history/2026-07-20-YR-074-반사실미세조정-prereg.md)·[YR-077](../docs/strategy-history/2026-07-20-YR-077-돌발강건성-prereg.md).
- **YR-075-a 재조작 목적지 (2026-07-20) — 배포형 규칙 H1 채택, 단 "RL 불필요" 결론은 재검토로 철회**: H1(관측 ETA 인지)이 greedy 대비 혼잡 국소 개선(채택본 표준 목적지 `adopted_slot_selector`). **⚠️ 신뢰성 재검토 (병렬 세션 적대 검토 → 본 세션 확증)**: 0단계 "오라클"은 비용 상한이 아니라 future_blocked 최소화 프록시(목적함수 미평가)·H1==오라클은 동일코드 우연(옆 셀서 잡음이 완전정보 이김)·**이동은 총비용의 1%**(지배=lane_cong 46%+truck_wait 37%)라 배치는 하류로만 간접 전달 → **"H1==천장·RL 무의미"는 미증명으로 격하.** 진짜 비용 최적 배치 헤드룸은 rollout 오라클(YR-075-c)로 재판정 대상. 함의 "정보는 활용할 결정이 있을 때 값을 함"(ETA 축)은 유지. [설계+결과](../docs/strategy-history/2026-07-20-YR-075a-재조작목적지-0단계-prereg.md)·[**신뢰성 재검토**](../docs/strategy-history/2026-07-20-YR-075a-신뢰성재검토-오라클상한성반박.md).
