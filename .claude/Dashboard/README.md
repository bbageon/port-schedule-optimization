...
- **현재 정책 후보 구조**: YR-087에서 feed-forward 증류는 미래 rollout을 재현하지 못해 공식 채택 경로에서 빠졌고, 현재 성능 선두는 H1 목적지 규칙을 쓴 **2크레인 중앙 3600초 공동 rollout**이다. 실행은 resolver가 안전·물리 제약을 보장한다. 다만 이 후보의 트럭 성과도 구 `S-B` 지표이므로 YR-089 전에는 최종 정책으로 동결하지 않는다. QMIX는 “다중 블록이라서” 자동 재개하지 않는다. 한계비용 quote가 공유자원의 비가산 효과를 반복적으로 놓치고 직접 공동평가가 감당되지 않을 때만 별도 비교한다.
