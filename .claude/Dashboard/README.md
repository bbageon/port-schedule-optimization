...
- **남은 실행 순서 (정정 트랙, 2026-07-19 사용자 판단 반영)**: BC 계열 제외 — ① **차분 × state_norm 결합** (차분 최적 창 2400s 에 입력 정규화 얹기 — 두 검증된 이득의 합산 여부) ② **차분 표적 QMIX** (개인 신호를 mixer 표적으로 — 암묵 분해 실패·명시 분해 성공 패턴의 교차점, 설계·사전등록 필요) ③ YR-066(차분 잔여 용의자, 수확 체감). 병행 결정: YR-002 협약 트랙. 이후 YR-042 재개 → YR-014. 실측·validation 은 YR-002/009. **열린 문제·대응 종합**: [열린문제 전략](../docs/strategy-history/2026-07-16-열린문제-대응전략.md).
