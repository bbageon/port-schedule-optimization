...
- **남은 실행 순서 (정정 트랙)**: **다음 = 사용자 결정 2건** — ① YR-013 QMIX 재판정(재감사가 "조정 실패=QMIX가 겨냥하는 문제"로 특정 — 착수 여부) ② YR-052 전략 WAIT 제외(데이터상 권고 확정). 결정 전 실행 가능한 것은 YR-056(경량 협조 feature 설계). 이후 YR-042 재개 → YR-014. 실측·validation 은 YR-002/009. **열린 문제·대응 종합**: [열린문제 전략](../docs/strategy-history/2026-07-16-열린문제-대응전략.md).
