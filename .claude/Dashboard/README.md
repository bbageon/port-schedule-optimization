...
- **YR-075-a 재조작 목적지 확장 (2026-07-20) — 게이트형 완결: 배포형 규칙 H1 채택, RL 불필요 확정**: 오라클 천장은 고혼잡(fill0.70)에만 −8% → 관측 ETA·마감만 쓰는 배포형 H1 이 그 오라클을 100% 잡음(채택본 −27·~10% 유의)·저혼잡 중립·다운사이드 0 → **H1==오라클로 K-후보 RL 격차 0 = 생략**, 규칙 교체만으로 이득. 채택본 표준 목적지 = H1(`adopted_slot_selector`). **함의: "완벽 ETA 가치 0"(YR-070/71/72)은 목적 탓 아닌 "행동공간에 ETA 쓸 자리 없음"이었고 — 목적지 결정을 열자 ETA 가 값을 함 (예측 적중, 정보는 활용할 결정이 있을 때만 값을 함).** 반입 배치(YR-075-b)는 생애주기 모델 선행·PoC 이후. [설계+결과](../docs/strategy-history/2026-07-20-YR-075a-재조작목적지-0단계-prereg.md).
