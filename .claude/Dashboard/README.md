...
- **현재 정책 후보 상태 (2026-07-27)**: In Progress와 Ready는 비어 있고 **공식 배포 후보도 아직 없다**. 다음 재배정 흐름은 YR-103에서 3~7분 도착·동적 혼잡의 공개정보 기대상금을 확인한 뒤, 양수일 때 YR-102 저분산 비용분해 quote로 간다. 별도 후보는 YR-096의 현실형 @3600 재검이다. YR-100은 계산계약 완료이지 정책 승리가 아니다. 보존 증거는 트럭 RL의 과거 `S→B` 개선, YR-089의 rollout `B→C` 상금, 적하 타이밍 여지(YR-098), 목적지 상금(YR-075-c), 제한된 오라클 재배정 상한(YR-099)이다. 새 후보 뒤 YR-041을 재집행하며 QMIX는 다중 블록이라는 이유만으로 재개하지 않는다.
