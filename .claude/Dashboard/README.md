...
- **재배정 트랙 현재 순서 (2026-07-28)**: YR-099-b 원자 이송 브리지와 YR-112/116 교착·리뷰창 계약은 완료됐다. YR-113은 현행 혼잡격차 0.10 이송이 이송 0보다 트럭 비용을 낮춘다는 효과를 독립 두 대역에서 확증했다. 현재는 **YR-105-b**에서 `0.05/0.10/0.20` 중 언제 이송할지를 한 축으로 최적화한다. 그 뒤 채택 임계 기준으로 **YR-115 총비용 1차 확증**, 이후 필요할 때만 YR-102 quote 정교화로 간다. 절대혼잡·지속조건·본선필터를 동시에 섞지 않는다.
...
- **현재 정책 후보 상태 (2026-07-28)**: 공식 배포 후보는 아직 없지만 창중 반입 이송 자체의 양의 효과는 처음 확증됐다. 현행 0.10 규칙은 이송 0보다 truck 채널 `+3.35`만큼 유리했고, 새 후보·리뷰창 계약에서도 같은 106쌍 집계 결론이 유지됐다. 현재 YR-105-b가 임계 하나만 최적화 중이며, 승자가 나오면 별도 신규 대역에서 확증한다. YR-115는 그 채택 임계가 터미널 총비용도 줄인다는 문장을 1차 가설로 검증한다. QMIX·추가 관제층은 이 단일축 검증에 넣지 않는다.
