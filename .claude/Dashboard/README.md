...
- **PoC 범위 정정 (2026-07-21, 사용자)**: **1단계 = 블록 기준 트럭 대기 최소화 학습 정책 = 완료** (규칙 초과·강건성·H1 목적지까지). **본선(선박 정시성)은 결함이 아니라 계획된 2단계 확장** — 1단계는 의도적으로 트럭에 집중. 앞선 "총비용인데 본선 뺀 결함" 어투는 순서 오해 과표현으로 철회. 이번 세션 본선 진단(재판정·통제가능성·λ)은 2단계 사전조사로 재분류. 착수 선결 = 목적함수 총비용 정식 확정(YR-080·병렬). [정정 기록](../docs/strategy-history/2026-07-21-PoC범위-트럭우선-본선2단계-정정.md).
