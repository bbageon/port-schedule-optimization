...
## 사후 비용계약 변경 (2026-07-30)

사용자 결정으로 이 구현의 **계단형 트럭 비용(1→2/h)과 본선 최대 33/h는 v1 이력**으로
보존한다. 앞으로의 정책계약은 [YR-136](YR-136-smooth-cost-contract-v2.md)이 승계한다.

- 트럭: SLA 접근 마지막 10분 동안 smoothstep으로 `1→2/h`
- 본선 적하: 계획완료 접근 마지막 30분 동안 smoothstep으로 `0→10/h`
- 후보 지연비용: 끝점 비용률을 곱하지 않고 해당 시간구간을 적분

커밋 `322af31`과 기존 곡선 evidence는 수정하지 않는다. v2는 opt-in 신규 비용계약으로
구현하고, 트럭 단일축·본선 단일축을 분리 검증한 뒤 결합한다.
