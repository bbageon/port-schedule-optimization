...
| **0B-H / 0B-V 신호 재검사** | 구조별 지속 유입 | 실제 SELL 상금·공개정보 부호 구분력·결정 순간 수신 여유 | **각 구조에서 별도 GO/STOP** |
...
- 구조별 0B 통과 시에만 Q30 동결 → shadow → on-policy 학습 → K/Q30/S 3군. H 통과가 V 통과를 대신하지 않는다.
