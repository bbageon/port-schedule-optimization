...
`derive_next_scope()` 신설. `GATE_DEPENDENCY_ORDER` 에서 **미해소 중 가장 앞선
축**을 유도하고 문자열 상수는 전부 제거했다.

| | |
|---|---|
| 구 안내 | `reliability PASS → YR-150: scenario_validity 단일축` |
| 신 안내 | `performance 단일축 — 규칙 기준선 대비 성능 판정 (미해소: performance)` |
| 판정 3축 | 재발행 전후 **동일** (INCONCLUSIVE/PASS/PASS) — 안내만 바뀜 |

시험 31/31 (신규 12 + 기존 19). 8조합 전수에서 **이미 통과한 축을 절대
가리키지 않음**을 검사한다.
