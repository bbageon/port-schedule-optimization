...
- 평균과 총 점유면적을 주판정으로 사용한다. P95 `B→C`·`A→O`는 트럭 100대 중
  오래 걸린 상위 5대 부근의 꼬리 진단값으로 보고하되 reward·후보선택·채택/비열등
  게이트에는 사용하지 않는다.
- 장기대기 보호는 `long_wait` 초과면적과 SLA 초과 건수·비율·초과시간으로 확인한다.
  실제 터미널의 P95 SLA와 충분한 표본이 확보될 때만 P95 하드 게이트를 재검토한다.
...
- SF-SPT·현재 rollout 후보를 신규 seed의 반입/반출별 P50·평균·P95 `B-C`와
  `A-O`, 서비스시간, 완주·본선 지표로 재평가한다. 이때 P95는 보고값이고,
  합격·불합격은 총비용·평균·본선과 안전/완주 guard로 판정한다.
