...
- pilot16으로 선택 34·확증 68을 동결했고 guard·계획 MDE90을 모두 통과했다.
- 선택 편익 `0.10−후보`: `0.05`는 total −2.505·A→O −0.093분,
  `0.20`은 total −1.007·A→O −0.434분.
- 두 후보 모두 공동 적격조건을 만족하지 못해 `NO_CANDIDATE`.
- 계약대로 확증을 열지 않고 현행 `0.10`을 유지한다.
- [결과 보고서](../../../outputs/reports/yr105b_transfer_threshold/report.md)
