...
**완료 당시 실측 (seed 310000, λ=1.0 중립, YR-043 정정 비용, ETA 주입 전)**:
...
1차 baseline 이 base 정책보다 총비용이 낮았다(93.4 < 96.6) — 정책개선이 실제로 작동했다.

**YR-048 ETA 주입 후 정정 (같은 seed, 2026-07-16)**:

| 정책 | 총비용 | 평균대기 | 완료율 | 실작업 가능시 SERVE 선택 | 조합 축소 |
|---|---:|---:|---:|---:|---:|
| ServiceFirstSPT | 79.643 | 0.390분 | 100% | 0.578 | 0 |
| JointRolloutGreedy | **62.927** | **0.345분** | 100% | 0.456 | 4 |

ETA는 PRE_REHANDLE뿐 아니라 REPOSITION 후보도 늘리므로 절대 수치는 바뀌었다. 하지만 rollout이
base 이하이고 건전성 문턱 0.25를 통과한다는 계약 결론은 유지된다. 조합 축소는 이제 횟수를 결과에
노출하며 YR-045가 seed별로 보고한다.
...
```
- **문턱 보정 근거**: 퇴화 **0.03~0.08** vs ETA 전 건전 **0.46~0.57**, ETA 후 건전
  **0.46~0.58** — 두 무리가 뚜렷이 갈리므로 그 사이 0.25를 문턱으로 둔다.
...
조합 축소 계수·결과 노출 · 전 정책 동일 제약·완주. `tests/unit/test_candidate_generator.py`:
mandatory 전량 보존(K 확장)·non-mandatory budget 준수.

YR-048 후 재검증: 관련 ETA+baseline **16 passed**, Windows 순수 파이썬 **287 passed**,
WSL torch 포함 **328 passed·3 skipped**.
...
> 환경 후속: YR-046이 Windows 순수 파이썬 + WSL torch의 2원 실행환경을 복구했다. YR-045는
> WSL torch 전체 검증과 clean commit을 선결조건으로 사전등록했다.
