...
2. **BEAM 이 JointRollout 과 60/60 seed 완전 동일** — 2단 lookahead 가 결정을 한 번도 바꾸지
   않음. → **YR-055 (2026-07-18) 결함 확정·수정**: `_tail` 이 pending 결정에서 조기 반환해
   전 분기 tail≈0. 본 결과의 BEAM 열은 JointRollout 복제로 읽어야 하며 게이트 판정에는 무영향
   (기준 baseline 아님). [상세](../YR-055-BEAM-tail-결함.md).
