...
1. YR-050이 ETA 기반 결정 시점과 연착 ETA feature 의미를 테스트로 고정한다.
2. Windows 순수 파이썬 전체와 WSL torch 테스트가 통과한다.
3. 실행 tree는 clean commit이며 manifest에 commit·dirty=false를 기록한다.
4. 비용 scale은 train 기준정책으로만 맞춘 뒤 validation/test에서 재계산하지 않는다.
