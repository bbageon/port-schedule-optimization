...
> 기록일: 2026-07-15 · 상태: **완료 — H-A SUPPORTED (0.852) · H-B REJECTED (이득 0.000)** · 사용자 승인: "YR-031-b 반영하고 착수해봐" · 사전등록 원문(§1~§6) 불변, 결과는 하단 append
...
- 실행: clean source `7959798` (1차 실행은 콘솔 em-dash cp949 크래시로 중단 —
  progress 문자열만 수정, 연산 무변경). 결과 커밋 `824e9e7` ·
  [리포트](../../../outputs/reports/oracle_pattern_hjnc/oracle_pattern_report.md)

### 판정

| 가설 | 지표 | 임계 | 판정 |
|---|---|---|---|
| **H-A** 이탈 시점의 feature 예측가능성 | day-그룹 CV AUC **0.852** (n=10,000 결정, 이탈 416) | ≥0.75 | **SUPPORTED** |
| **H-B** 이탈 선택의 집합 의존성 | 쌍별 AUC **0.993** → +집합맥락 0.993 (이득 **+0.000**) | ≥0.05 | **REJECTED** |

사용자 가설의 전반부("패턴 속 최적화 포인트가 존재하고 찾을 수 있다")는 강하게
지지, 후반부("액션/스코어링 구조가 한계")는 기각 — **후보 독립 argmin 골격은
충분하며, 부족한 것은 입력 정보다.**

### 이탈의 정체 — "근소 동급 작업 간 미세 스왑" (tie-region 정련)

- 이탈률 4.2% (416/10,000), 즉시 희생 중앙값 0.0038분/결정 — 희소·저비용 분기.
- **90% anti-SPT** 인데 oracle−greedy service 차이 중앙값이 **+9.5초**뿐 (대기
  차이 −0.7초) — "긴 작업 우선"이 아니라 **거의 같은 길이의 두 후보 중 greedy
  의 엄격한 SPT 순서를 뒤집는 것**. greedy 의 손실은 큰 오판이 아니라 동급
  구간에서의 순서 무감각.
- 층화: 이탈 빈도는 혼잡도에 둔감(3.8/4.2/4.5건/일)하나 **상금의 50%가 혼잡
  상위 1/3 에 집중** — 같은 스왑이 혼잡일에 더 비쌈.

### H-A 신호 운반체 (사후 전수 fit 가중치 — 기술통계, 판정과 별개)

| feature | 가중치 | 비고 |
|---|---|---|
| svc_mean (집합) | −1.41 | 대기열 평균 작업이 짧을수록 이탈 |
| 짧은작업비율 (미래) | +1.31 | 잔여 작업이 짧은 것 위주일 때 |
| 반출비율 (집합) | +1.19 | 반출 중심 국면 |
| svc_max (집합) | +0.94 | 집합에 유난히 긴 작업이 섞여 있을 때 |
| 최근접잔여bay (미래) | −0.81 | 잔여 작업이 가까울 때 |

**상위 신호 5개 중 4개가 집합 맥락(8종)** — 현 Δ-net 의 14-input 에는 이
집합 feature 가 없다 (전역 5·후보 5·미래 4 뿐). H-A 의 예측력은 Δ-net 이
못 보는 정보에서 나온다.

### 파생 결정

1. **YR-012-c 신설**: Δ-net 입력 14→22 (집합 맥락 8 추가 — H-A 운반체).
   구조는 불변 (H-B 기각 — 집합 인코딩·결정시 탐색 불필요). online TD 유지
   (YR-012-b 교훈). 상금의 회수량이 본 판정의 후속 측정.
2. 사용자 가설 중 "보상 체계 단순함" 부분은 사전 §1 대로 용의자 제외 유지 —
   본 결과도 보상이 아니라 정보 부족을 지목.
3. divergence_events.json (416건 원자료)은 YR-012-c 의 feature 검증·후속
   분석 재료로 박제.
