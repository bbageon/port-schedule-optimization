...
> 기록일: 2026-07-13 · 상태: **전략 동결, 미구현·미검증**
> 후속 구현·평가 상태: [Dashboard YR-027](../../Dashboard/backlog.md)
...
`ĉ_t(j) = (q_t-1) × estimated_service_time(j) / (60N)`,
`a_t = argmin_j ĉ_t(j)`.
...
현재 `run-exp1-cost`는 선박 기본 8건, 9개 휴리스틱 rule action, `reward=-C_won`, `γ=0.98`의
기존 hyper-heuristic이다. 현재 rule executor의 FIFO는 gate 시각을 쓰고 공통 tie-break에는 SLA와
deadline이 숨어 있으므로 그대로 재사용하지 않는다. YR-027에서 별도 환경·동적 Job action·Cost-Q
min update·fallback·전용 baseline 테스트를 구현한 뒤에만 이 전략을 검증했다고 한다.
