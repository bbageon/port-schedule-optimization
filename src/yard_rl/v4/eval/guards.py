...
#: 9_400_000 은 v3 최종 판정([[YR-233]] · 2026-08-30, `outputs/v4/judge-locked/`)에서
#: 썼다. 한 대역은 한 번만 쓴다 — 두 번째 판정은 그 대역에서 고를 여지를 만든다.
#: 9_500_000 은 동의 절제 확증([[YR-296]] · 2026-09-04, `outputs/v4/judge-consent/`).
#: 9_000_000 은 크레인 판정([[YR-314]] · 2026-09-15, `outputs/v4/judge-crane/`)에서 썼다.
#: ★그 판정은 **하드가드(전건 투입)에서 실격**됐다 — 28일 중 13일에서 블록 용량 부족으로
#: 트럭이 거절돼 `admitted < load`. 결과는 무효이지만 **시드와 수치가 노출됐으므로**
#: 대역은 소진된 것으로 다룬다(열람 대역 재사용 금지). 재판정은 새 대역에서 한다.
#: 8_900_000 은 크레인 재판정([[YR-316]] · 2026-09-16, `outputs/v4/judge-crane-2/`) —
#: 용량 인식 무대에서 새 망으로. 가드 전부 통과(전건 투입 · 예외 0 · rollout 0 · 코드 청결).
USED_BANDS = (8_900_000, 9_000_000, 9_200_000, 9_300_000, 9_400_000, 9_500_000,
              9_600_000, 9_700_000, 9_800_000)
#: 비판정(진단) 시드 대역
