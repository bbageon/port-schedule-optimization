"""v2 — Q 비용 채점 정책이 보는 특징 — **이 세대 전용 사본**.
...
■ ⚠️ 알려진 결함 (v3 축 ④ 가 고친다 — YR-208)
  1. **시각이 없다** — 무대에 아침·저녁 이중 피크가 있는데 정책에 시계가 없다.
  2. **대기 대수 정의가 채점과 다르다** — 여기서 주는 `inside` 는
     `gate_in ≤ t < gate_out`(블록 안 전부)인데, 보상은 `_waiting_at`
     (`block_arrival ≤ t < service_start`, 줄 선 것만)을 쓴다.
...
#: KEEP(안 팔기)의 고정 기준점 — 학습하지 않는다.
#: 다른 좌표의 비용을 여기에 상대적으로 재므로 눈금의 원점이다.
KEEP_Q = 0.0
