...
#:   있는데 **배울 결과가 없었다.**
#:
#: ■ 왜 470초인가 — ★400초는 틀린 목표였다 (2026-09-07 게이트 실패로 정정)
#:   처음엔 400초를 골랐다. 공급 간격 133.3초가 STS 수요 130.9초와 "거의 같으니
#:   빠듯해서 굶겠다" 고 본 것인데, **균형점은 굶는 게 아니라 딱 맞춰 도는 것**이다.
#:   STS 가 트랙터에 발이 묶이면 그 이동간격이 곧 STS 간격이 되므로
#:
#:       유휴 비율 = (공급간격 − STS간격) / 공급간격
#:
#:   400초면 (133.3−130.9)/133.3 = **1.8%** 에 그친다. 게이트 실측이 정확히
#:   1.00% 로 나와 이 계산과 맞았다 — 모형은 정상이고 목표값이 틀렸다.
#:
#:   470초면 공급 156.7초로 유휴가 **16.4%** — 문헌의 안벽 성능 손실 10~20% 한가운데다.
#:   왕복 약 8분도 현실 터미널(5~10분) 안이다.
#:
#: ■ 트럭 축은 거의 안 건드린다
#:   이 fleet 은 **안벽↔야드 전용**이다(`_transfer_request` 는 STS 양하와 적하 반출
#:   완료에서만 불린다). 외부 트럭은 게이트에서 YC 가 직접 상대하므로 경로가 다르다.
#:   400초 실측에서 90분위 체류가 4.3→4.4h(1.03배)에 그친 것이 그 증거다.
#:
#: ⚠️ v3 는 180초 그대로다. **두 세대의 무대가 다르므로 성능 비교 불가.**
YT_ROUND_TRIP_S = 470.0
