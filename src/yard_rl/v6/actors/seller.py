...
class Seller:
    ...
    def __init__(self, net: SellerNet, layout, *, explore: float = 0.0,
                 seed: int = 0):
        self.net = net
        self.selector = None  # v5 PPO: shared block policy; legacy path stays available.
        self.layout = layout
    ...
    def decide(self, mbt, src: str, *, doc_key: str, order, rec, t: float,
        ...
        if self.selector is not None:
            if forced is not None or self.explore:
                raise ValueError("PPO cannot mix counterfactual forcing or epsilon exploration")
            cost = torch.zeros(len(rows))  # No KRW prediction / resolver preference.
            idx = self.selector("seller", src, t, x)
        else:
            with torch.no_grad():
                cost = self.net(x)
        if self.selector is not None:
            pass
        elif forced == SELL and len(meta) > 1:
