...
#: ★바닥 비교용 **짧은 무대** (사용자 결정 2026-08-28) — 9일을 만들고 앞뒤를
#: 버려 **7일을 학습**한다. 30일 × 바닥 5종 = 89시간이 너무 비싸서 줄인 것이다.
#:
#: ■ 왜 부하 대역을 다 덮나
#:   3,500~7,500 만으로 줄이자는 안이 먼저 있었으나, Φ 가 부하에 극단적으로
#:   민감하다 — 부하 3,500 이 8,123만원인데 15,000 은 32억(40배)이다. 혼잡을
#:   빼면 **재배치가 가장 값어치 있을 지형을 한 번도 못 배운다**. 그러고 나서
#:   판정을 30일(혼잡 포함)로 하면 배운 적 없는 부하에서 시험보는 셈이 된다.
#:   그래서 혼잡·초혼잡을 하루씩 넣었다.
#:
#: ■ 앞뒤 3,500 은 **연결용**이다 (`is_train` 이 걸러낸다)
#:   야드 상태를 이어 주는 것이 목적이라 한산한 날이 낫다.
#:
#: ⚠️ 이 무대로는 **판정을 못 한다** — 부하당 1~2일이라 부호검정 표본이 없다.
#:    학습 전용이고, 판정은 새 대역 30일에서 따로 한다.
SHORT_LOADS: tuple[int, ...] = (
    3_500,                                                    # 연결용
    3_500, 5_000, 5_000, 7_500, 12_500, 15_000, 3_500,        # ← 학습 7일
    3_500,                                                    # 연결용
)


def plan_days(seed: int, loads=SHORT_LOADS, *, weights=LOAD_WEIGHTS) -> list[DayPlan]:
    """부하를 **직접 지정**해 계획을 만든다 — `plan_month` 의 추첨 대신.

    바닥 5종을 견주려면 **다섯이 똑같은 달**을 봐야 한다. 추첨은 시드만 같으면
    같은 달을 주지만, 부하 구성을 손으로 정하고 싶을 때(대역을 다 덮게) 쓴다.
    `seed`·`t0` 규칙은 `plan_month` 와 같아서 아래 단계는 아무것도 안 바뀐다.
    """
    names = {w[0]: w[2] for w in weights}
    return [DayPlan(index=i, load=int(v), label=names.get(int(v), f"{int(v):,}"),
                    seed=seed + 1_000 * (i + 1), t0=i * DAY_S, n_days=len(loads))
