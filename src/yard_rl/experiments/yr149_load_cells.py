...
■ 자격 검사(**하드 8종** — qualify() 의 hard dict 와 1:1): ①A 중첩 A50⊂A75⊂A100⊂A125⊂A150
  ②A 유지 작업 필드 바이트 동일 ③B 5셀 동일 ④A 초기 적재 지문 동일 ⑤A 본선 지문 동일
  ⑥정확 개수 ⑦정책 예외 0 ⑧장부 항등(A 등록 수 = m + B_SIZE).
  **검증력 주의(31차)**: ③④⑤는 make_a_cell 이 master 의 containers·vessels 를 그대로
  물려주고 B 를 m 과 무관하게 재생성하므로 **구조적으로 실패할 수 없다** — 부하 변경이
  배치·본선을 건드리지 않았다는 독립 증거가 아니라 **생성기 결정성 검사**다. 실행 측
  불변성은 by_cell 의 B 지표(가동률·A→O)가 5셀에서 같은지로 별도 확인한다.
