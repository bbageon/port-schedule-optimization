...
class Buyer:
    ...
    def __init__(self, net: BuyerNet, *, explore: float = 0.0, seed: int = 0):
        self.net = net
        self.selector = None  # v5 PPO: same policy instance as seller and cranes.
    ...
    def respond(self, mbt, offer: Offer, *, order, rec, t: float,
        ...
        x = torch.tensor([row_buy, row_reject], dtype=torch.float32)
        if self.selector is not None:
            if self.force_once or self.explore or self.always_buy:
                raise ValueError("PPO cannot mix forced responses or epsilon exploration")
            idx = self.selector("buyer", own, t, x)
            cost = torch.zeros(2)
        else:
            with torch.no_grad():
                cost = self.net(x)
        ...
        if self.selector is not None:
            action = (BUY, REJECT)[idx]
        elif forced in (BUY, REJECT):
