...
def run_month(*, seed: int, arm: str = "RL", seller_net=None, buyer_net=None,
    ...
    """
    if ppo is not None and (arm != "RL" or labels_per_day or explore or
                            explore_of_day is not None or on_fit is not None or
                            seller_net is not None or buyer_net is not None):
        raise ValueError("PPO uses one shared policy, without CF labels or legacy training")
    if arm not in ARMS:
    ...
    ctx = _MonthCtx(seller_net=(None if ppo else seller_net if seller_net is not None else SellerNet()),
               buyer_net=(None if ppo else buyer_net if buyer_net is not None else BuyerNet()),
    ...
    if ppo is not None:
        market.seller.selector = market.buyer.selector = ppo.select
    ...
    if ppo is not None:
        ppo.bind(mbt, bridge, meta, archive)
    ...
    if ppo is not None:
        exec_policy = ppo.execute
    mbt.run(exec_policy, review_fn=review)
    if ppo is not None:
        bridge._sync(mbt, sim_end)
        ppo.finish(sim_end, terminated=False)  # Finite demand schedule is a time limit.
