...
# ★6차 계약 (사용자 지시 2026-08-26) — 24시간 연속 운영 반영
#   이력: .claude/docs/strategy-history/2026-08-26-도착곡선-6차계약-야간38.md
#   근거: 부산신항은 **게이트가 24시간 돈다**. 5차 계약의 야간 15% 는 야간 물량을
#         과소평가했다 — 밤에도 반출입이 끊기지 않는다(사용자 확인).
#   봉우리 셋: 오전(10시) · 오후(15시) · ★저녁(21시, 신규 보정항)
#   가중은 **봉우리끼리의 상대값**이다. 총량의 38% 는 야간 바닥이 먼저 가져간다.
DIURNAL_PEAKS = ((10.0, 1.5, 0.317),    # 오전 — 08~11시 지배 · 68% 가 08:30~11:30
                 (15.0, 2.5, 0.633),    # 오후 — 12~18시 지배 · 68% 가 12:30~17:30
                 (21.0, 1.0, 0.050))    # ★저녁 — 19~23시 보정 · 지배하지 않는다
DIURNAL_NIGHT_FRAC = 0.38        # 야간 바닥 ÷ 일평균. **24시간 전부에 균등**하게 깔린다
#   ⚠️ "야간" 은 22~06시에 봉우리 항이 0 이라 이것만 남기 때문에 붙은 이름이다.
#      낮 12시에도 이 바닥은 그대로 깔려 있다 (부하 12,500 기준 시간당 198대).
# ★하루 물량 (사용자 결정 2026-08-20): 3,600 → 7,500
#   평균 312.5대/h · 피크 1,018대/h. 크레인 능력은 21블록×2대×(3600/180s) = 840대/h 라
#   **피크에 121% 로 초과**한다 — 아침·저녁에 줄이 쌓였다가 밤에 빠지는 무대가 된다.
#   하루 총량 7,500 은 하루 능력 20,160 의 37% 이므로 결국 다 처리된다.
#   ⚠️ 과거 판정 수치(3,600 무대)와 **직접 비교 불가**. 판정은 짝비교라 규칙 팔을
#   같은 런에서 다시 재므로 앞으로의 판정 자체는 성립한다.
DIURNAL_DAY_TOTAL = 7_500        # 하루 총 도착 대수 (평균 312.5대/h)
...
# ══════════════════════════════════════════════════════════════════════════════
# v3 무대 축 — 하루 물량 3수준 · 통지 리드타임 분포
# 정본: `.claude/docs/architecture/02-무대.md` §2-1 · §4
#
# ★둘 다 **덧붙이기만** 한다. 기존 상수·기본값을 건드리지 않으므로 v1·v2 실행은
#   글자 그대로 같은 결과를 낸다 — 세대 비교가 짝비교라 무대가 흔들리면 안 된다.
# ══════════════════════════════════════════════════════════════════════════════

#: 하루 물량 3수준 — **셋 다 실측이고 셋 다 통과해야** 한다 (사용자 결정 2026-08-20).
#: 한산(3,500) · 빡빡(5,000) · 피크 초과(7,500). 계약(`load_levels_target = 3`).
# ★2026-08-26 확장 (사용자 지시) — 혼잡 환경을 보려고 12,500·15,000 을 더했다.
# `v3/eval.LOAD_LEVELS` 와 **같아야** 한다 (아키텍처 교차검증이 대조한다).
DIURNAL_LOAD_LEVELS: tuple[int, ...] = (3_500, 5_000, 7_500, 12_500, 15_000)

#: 통지 리드타임 실측 분위 앵커 — (분위, 시간). DGT 실측 100건.
#: 중앙값 1.27h · 사분위폭 2.36h · **음수(통지 전 도착) 14.1%**.
#: 앵커 사이는 선형보간이다 — 100건으로 모수분포를 주장할 근거가 없어서,
#: 분포 형태를 가정하는 대신 **실측 분위를 그대로 잇는다**.
LEAD_TIME_DIST: tuple[tuple[float, float], ...] = (
    (0.000, -1.50),
    (0.141, 0.00),      # ← 여기 아래가 음수 리드 14.1%
    (0.250, 0.35),
    (0.500, 1.27),      # ← 중앙값
    (0.750, 2.71),      # ← 1분위 0.35 + 사분위폭 2.36
    (0.900, 5.00),
    (0.980, 10.00),
    (1.000, 24.00),
)
...
def sample_lead_s(u: float) -> float:
    """분위 `u`∈[0,1) → 통지 리드타임(초). 음수는 **0 으로 눌러 담는다**.

    음수 리드는 "통지보다 트럭이 먼저 온" 경우다. 라이프사이클이
    `copinoNotice ≤ gateIn` 을 요구하므로(00 §2) 0 으로 눌러 통지 즉시 도착으로
    다룬다 — 이 트럭은 **공간 판매가 불가하고 시간 이연만 가능**하다.

    난수를 안에서 뽑지 않고 분위를 **받는다**: 시드에서 미리 추출해야 재현된다.
    """
    ...
    for i in range(1, len(qs)):
        ...
        q1, h1 = qs[i]
        if u <= q1:
            w = 0.0 if q1 == q0 else (u - q0) / (q1 - q0)
            return max(0.0, (h0 + (h1 - h0) * w) * 3600.0)
    return max(0.0, qs[-1][1] * 3600.0)


def lead_time_negative_share() -> float:
    """음수 리드 비율 — 분포표에서 직접 읽는다(하드코딩 아님)."""
    for q, h in LEAD_TIME_DIST:
        if h >= 0.0:
            return q
    return 0.0
