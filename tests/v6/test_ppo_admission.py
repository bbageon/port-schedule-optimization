...
from yard_rl.v6.ppo import continuous
from yard_rl.v6.ppo.journal import AdmissionFailure, RunJournal
from yard_rl.v6.ppo.runtime import PPORuntime
from yard_rl.v6.stage import month_run
from yard_rl.v6.stage.month_run import MonthResult
...
def test_real_truck_admission_stops_before_policy_boundary(test_stamp, monkeypatch, tmp_path, failure,
    ...
    def bad_schedule(*args, **kwargs):
        ...
        e['target'] = (sorted(built['day0']['scenarios'][e['block']].containers)[0]
                       if failure == 'no_target' else None)
        e['con_no'] = e['target'] if failure == 'no_target' else 'IN_' + e['job_id']
        if failure == "tail":
...
def test_real_vessel_failure_is_saved_with_identity(test_stamp, monkeypatch, tmp_path,
    ...
    captured = {}
    def fail_admission(mbt, bid, row, **kwargs):
        captured.update(row)
        raise TransferError('injected vessel admission failure')
    monkeypatch.setattr(month_run, 'inject_vessel', fail_admission)
    out = tmp_path / "vessel"
    with pytest.raises(AdmissionFailure, match='injected vessel admission failure'):
        continuous.run_continuous(output=out, n_days=3, load=1)
    status, admissions = read(out, "status.json"), read(out, "admissions.json")
    ...
    assert row['asked'] == captured['moves'] and row['moves'] == 0
