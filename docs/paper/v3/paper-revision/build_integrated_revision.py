...
all pending work between them, and the feasible move-aside reposition
the generator emits was discarded once higher-scoring blocked jobs
filled the candidate budget, idling the block for as long as 23.5
days. These runs, not the policies, drive the means;
...
The repair, ranking feasible candidates ahead of blocked ones, is
implemented and tested; it alters the candidate set whenever the budget
binds, so the re-run needs fresh pre-registration. The deadlock-free rows are a validity filter, not a registered
...
ranking agreement. Online latency is likewise unmeasured: candidate construction, scoring, conflict
...
"""
    ...
    print(json.dumps(dict(references=len(cited),
        submission_ready=receipt['submission_ready'], output='main-integrated.tex')))
