...
**Result.** The 80 runs are complete (Table 3, §5.4). Time-only was cheaper than no reallocation in 16/20 months (median 12.7%); Full in 4/20 and Block-only in 3/20; every pre-registered mean-difference interval includes zero. Nine runs in six seeds, two of them baseline runs, ended in a simulator crane deadlock that inflates monthly cost 5–10 fold. The cause is candidate selection, not a missing action: two idle cranes sit within the safety gap at a block end with all pending work between them, the generator does emit a feasible move-aside reposition, and the fixed candidate budget discards it because blocked jobs score higher. The repair (ranking feasible candidates ahead of blocked ones) is implemented and tested, and because it alters the candidate set whenever the budget binds it needs a fresh pre-registration rather than a re-run of these months. On the 14 seeds where no arm stalled for 24 h or more — a mechanical criterion applied to every policy, so all arms share one sample — Time-only is cheaper in 14/14 months with a 97.5% interval of [+1.24, +2.07] billion KRW, while Full and Block-only are significantly more expensive. That subset is not neutral: the six removed seeds average 7.2% more requested trucks, so the filter discards the busiest months. Per-seed costs: [32-독립시드별-결과표.md](32-독립시드별-결과표.md); deadlock diagnosis: [31-정지-결함-진단.md](31-정지-결함-진단.md).
...
**Outstanding.** The deadlock repair is implemented and tested but changes the candidate set whenever the budget binds, so the affected months cannot simply be re-run under the old pre-registration; the deadlock-free rows are a validity filter, not a registered analysis. Unfinished work remains higher under every learned policy, and independent training replication has not been performed.
...
|---|---|---|
| E1 | [독립 반복 계약](20-독립반복-실행계약.md), [원래 사전등록](../../../../outputs/reports/yr317_v3_independent_eval/prereg.md), [80건 결과](30-독립반복-최종결과.md), [시드별 표](32-독립시드별-결과표.md), [교착 진단](31-정지-결함-진단.md) | 80건 완료·사전등록 통계 산출(표 3). 시간정책 16/20·네 팔 공통 14시드에서 14/14 구간 양수, 전체·블록 정책은 재현 안 됨. 교착은 후보 선택 결함이며 수정은 완료, 재실행은 새 사전등록에서 |
...
| E6 | [기존 부하별 근거](01-독립반복-부하별효과.md), [기존 성능표](08-성능비교표.md) | 과거 단일 실행의 성능을 조건별로 보존. 독립 실험(E1)에서 공간 조정 이득은 재현되지 않아 부정적 결과로 기술 |
...
- 현재 위치 표시는 9/21 PDF(표 3 포함, 12쪽)와 일치한다(문서 검사가 절·쪽을 대조).
- 독립 실험 최종 표·구간은 반영됐다. 교착 제외 줄은 유효성 필터이며 사전등록 표를 대체하지 않는다고 표시한다.
