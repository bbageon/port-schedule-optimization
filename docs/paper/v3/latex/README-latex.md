...
## 1. 한글이라 XeLaTeX 를 써야 한다

`pdflatex` 로는 한글이 깨진다. **`xelatex` 또는 `lualatex`** 를 쓴다.

```bash
xelatex main.tex
xelatex main.tex      # 상호참조·그림번호 확정을 위해 두 번
```

`main.tex` 는 `kotex` + `fontspec` 으로 `Noto Serif KR` 을 부른다. 그 글꼴이 없으면
아래 두 줄을 바꾸면 된다.

```latex
% \setmainhangulfont{Noto Serif KR}[AutoFakeSlant]
\setmainhangulfont{Malgun Gothic}      % 윈도우 기본 한글 글꼴
```

## 2. 보는 방법 — 셋 중 하나

이 저장소에서는 MiKTeX의 `xelatex`로 빌드를 검증했다.

### ① Overleaf — 설치 없이 가장 빠름 (권장)

1. <https://overleaf.com> 에서 **New Project → Upload Project**
2. `v3/figures/`와 `v3/latex/`를 같은 구조로 묶어 올린다
3. Main document를 `latex/main.tex`으로 정하고 Compiler를 **XeLaTeX**로 바꾼다
4. Recompile 하면 오른쪽에 PDF 가 나온다

한글 글꼴이 Overleaf 에 있으므로 `Noto Serif KR` 그대로 쓰면 된다.
...
```bash
sudo apt update && sudo apt install -y texlive-xetex texlive-lang-korean texlive-latex-extra
