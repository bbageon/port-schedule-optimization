...
슬라이드 **열네 장**(본편 열두 장 + 부록 두 장)을 장당 PNG 한 개로 만든다. 발표 프로그램에 그림으로 끌어다
...
| `curvedata.py` | (부록 공통) | 도착 곡선·시드 은행 자료를 **코드와 기록에서 직접 읽는다** |
| `slide_13_curve.py` | 부록 13 | 시드를 만들 때 쓰는 하루 도착 곡선 — 바닥 하나에 봉우리 셋 |
| `slide_14_load.py` | 부록 14 | 날마다 물량이 다르다 — 다섯 중 하나를 추첨한다 |

`slides/contact-sheet.png` 은 열네 장을 한 장에 모은 것이다. 흐름을 한눈에 볼 때 쓴다.
...
| 도착 곡선(바닥 38%·봉우리 10/15/21시) | `src/yard_rl/v3/world/integrated/terminal_stream.py` — 부록 두 장은 이 함수를 직접 불러 그린다 |
| 부하 추첨 확률과 실제 600일 비율 | `outputs/reports/yr318_seed_bank/run-d83c082/` (확증 캠페인 입력, 20달 × 30일) |
